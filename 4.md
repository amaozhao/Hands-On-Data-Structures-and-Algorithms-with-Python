...
假设我们已经有了一个包含```eggs```、```ham```、```spam``` 三项的链表，下面的代码是执行删除操作，即从链表中删除一个值为 ```ham``` 的数据元素 给定链表：
...
向双向链表插入元素的最坏情况时间复杂度为 $O(1)$，因为我们已经有了指向链表末尾的尾指针，我们可以直接添加新元素。 接下来，我们将讨论如何在双向链表的中间位置插入节点。
